...
def test_welcome_page_contains_animated_visual_and_language_card(qapp):
    window = installer.InstallerWindow(lang="sk", theme="light")
    try:
        assert window.size().width() == 920
        assert window.size().height() == 700
        assert window.windowFlags() & installer.Qt.WindowType.FramelessWindowHint
        ...
        assert window.welcome_visual._animation.state() == QAbstractAnimation.State.Running
        window.theme = "dark"
        window._apply_theme()
        qapp.processEvents()
    finally:
...
def test_installer_controls_and_step_badges_are_solid_and_unclipped(qapp):
    window = installer.InstallerWindow(lang="sk", theme="light")
    try:
        assert list(window.window_controls) == ["maximize", "minimize", "close"]
        controls = list(window.window_controls.values())
        assert [(item.width(), item.height()) for item in controls] == [(10, 10)] * 3
        assert [item._color.name().upper() for item in controls] == [
...
def test_installer_route_visual_animates_on_its_own_confirmed_path(qapp):
    visual = installer.InstallerWelcomeVisual(lambda: "light")
    visual.resize(310, 200)
    visual.show()
    try:
        visual.phase = 0.05
        qapp.processEvents()
